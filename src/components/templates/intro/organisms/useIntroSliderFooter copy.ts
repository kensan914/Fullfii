...
export const useIntroSlideFooter = (
  currentPage: number,
  pageLength: number,
  currentPageSetting: IntroPageSetting,
  currentIsReadyFooter: boolean,
  ...
  const onPressBottom = () => {
    ...
    // 次ページのisReadyFooterがfalseの時のみFADE_OUTアニメーション
    if (!isReadyFooterCollection[currentPage]) {
