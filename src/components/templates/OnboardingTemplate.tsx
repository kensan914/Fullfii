...
export const OnboardingTemplate: React.FC = () => {
    const slides = [
        {
          key: 'one',
          title: 'Fullfiiとは',
      ...
      const renderItem = ({ item }) => {
        return (
        <Block flex  style={[styles.container, {backgroundColor: item.backgroundColor}]} >
            <Block center style={styles.imageContainer}>
                <Image style={styles.image} source={item.image} />
            </Block>
            <Block style={styles.subContainer}>
            <Block center style={styles.titleContainer}>
                <Text size={32} bold color={item.textColor} center>{item.title}</Text>
            </Block>
            <Block center style={styles.textContainer}>
                <Text size={18} color={item.textColor} center >{item.text}</Text>
            </Block>
            </Block>
          </Block>
        );}
