...
export const ToTalkUserListItem: React.FC<Props> = (props) => {
    const {
        name,
        image
    } = props
    return(
        <Block row space="between" style={styles.listItemContainer}>
            ...
            <Button shadowless={true} color='transparent' opacity={0.6} style={styles.toSeeTalkHistoryBox}>
                <Block row center>
                <Text size={14} bold color={COLORS.BROWN} style={styles.textHeight}>
                    トーク履歴を見る
                </Text>
                <Block center style={styles.iconAngleRight}>
                    <IconExtra name="angle-right" family="font-awesome" size={32} color={COLORS.BROWN} />
                </Block>
                </Block>
            </Button>
        </Block>
